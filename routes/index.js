...
// Create a new address (insert a new saved address into the 'savedaddress' table)
router.post('/address/:addressId', async (req, res) => {
  const { addressId } = req.params; // Extract addressId from URL parameter
  ...
  try {
    ...
    res.status(201).json({
      addressId: result.insertId, // Return the generated addressId
      name,
      addressLine1,
      addressLine2,
      pincode,
...
router.get('/vendor/:vendor_id/items', async (req, res) => {
  ...
  try {
    ...
    const [items] = await db.query(
      'SELECT item_id, vendor_id, name, brand, size, color, item_condition, cost_price, selling_price, stock_quantity, imageURL, description, reviews FROM item WHERE vendor_id = ?',
      [vendor_id]
    );
...
router.get('/customer/:customer_id', async (req, res) => {
  ...
  try {
    // Fetch the customer details based on the provided customer_id
    const [customer] = await db.query('SELECT customer_id, name, email, phone, address, registration_date FROM customer WHERE customer_id = ?', [customer_id]);
...
router.post('/item', async (req, res) => {
  const { vendor_id, name, brand, size, color, item_condition, cost_price, selling_price, stock_quantity, imageURL, description, reviews } = req.body;

  try {
    // Insert a new item into the 'item' table
    const [result] = await db.query(
      'INSERT INTO item (vendor_id, name, brand, size, color, item_condition, cost_price, selling_price, stock_quantity, imageURL, description, reviews) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [vendor_id, name, brand, size, color, item_condition, cost_price, selling_price, stock_quantity, imageURL, description, reviews]
    );

    // Return the created item details along with the new item ID
    res.status(201).json({
...
router.get('/item/:item_id', async (req, res) => {
  ...
  try {
    ...
    const [item] = await db.query('SELECT item_id, vendor_id, name, brand, size, color, item_condition, cost_price, selling_price, stock_quantity, imageURL, description, reviews FROM item WHERE item_id = ?', [item_id]);
...
router.put('/item/:item_id', async (req, res) => {
  ...
  try {
    ...
    const [result] = await db.query(
      'UPDATE item SET vendor_id = ?, name = ?, brand = ?, size = ?, color = ?, item_condition = ?, cost_price = ?, selling_price = ?, stock_quantity = ?, imageURL = ?, description = ?, reviews = ? WHERE item_id = ?',
      [vendor_id, name, brand, size, color, item_condition, cost_price, selling_price, stock_quantity, imageURL, description, reviews, item_id]
    );
...
router.get('/items', async (req, res) => {
  try {
    ...
    const [items] = await db.query(`
      SELECT item_id, vendor_id, name, brand, size, color, item_condition, cost_price, selling_price, stock_quantity, imageURL, description, reviews
      FROM item
    `);
...
router.post('/signup', async (req, res) => {
  try {
    const { name, email, password, phone, address, registration_date } = req.body;

    // Check if email already exists
    const [existingCustomer] = await db.query('SELECT * FROM customer WHERE email = ?', [email]);
    ...
    const [result] = await db.query(
      'INSERT INTO customer (customer_id, name, email, phone, address, registration_date, password, cart_id, wishList_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', 
      [customer_id, name, email, phone, address, registration_date, hashedPassword, cart_id, wishlist_id]
    );

    // Return success message along with a generated token
    const token = generateToken(result.insertId);
    res.status(201).json({ id: result.insertId, name, email, token, customer_id, cart_id, wishlist_id });
